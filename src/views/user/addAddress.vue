<template>
  <div class="addAddress">
    ...
    <mu-form
      ...
      label-width="100"
    >
      <mu-form-item label="收件人" prop="recipients" :rules="perRuls" help-text="请填写收件人">
        <mu-text-field v-model="validateForm.recipients" prop="recipients"></mu-text-field>
      </mu-form-item>
      <mu-form-item label="联系电话" prop="mobile" :rules="mobileRules" help-text="请填写联系电话">
        <mu-text-field v-model="validateForm.mobile" prop="mobile"></mu-text-field>
      </mu-form-item>
      <mu-form-item label="所在地区" help-text="请选择所在地区">
        <mu-text-field @click="addressPicker" v-model="address"></mu-text-field>
      </mu-form-item>
      <mu-form-item label="详细地址" prop="street" :rules="streetRule" help-text="请填写详细地址">
        <mu-text-field
          multi-line
          :rows="3"
          :rows-max="6"
          v-model="validateForm.street"
          prop="street"
        ></mu-text-field>
      </mu-form-item>
    </mu-form>
    ...
    addressPicker() {
      ...
      weui.picker(dataList.dataList, {
        ...
        onConfirm: function(result) {
          ...
          vm.address =
            vm.validateForm.province +
            "-" +
            vm.validateForm.city +
            "-" +
            vm.validateForm.district;
        }
      });
    },
    save() {
      ...
      this.$refs.form.validate().then(result => {
        if (result) {
          if (self.address) {
            ...
            cmnUseraddressAdd(this.validateForm)
              .then(res => {
